...
// 查找新拍照
$stmt = $db->prepare("SELECT COUNT(*) FROM mir_photos WHERE created_at >= ?");
$stmt->execute([$last_check]);
$new_photos = (int)$stmt->fetchColumn();

// 查找新访问
$stmt = $db->prepare("SELECT COUNT(*) FROM mir_logs WHERE action='visit' AND created_at >= ?");
$stmt->execute([$last_check]);
...
$newest = null;
if ($new_photos > 0) {
    $stmt = $db->prepare("SELECT p.*, l.tags FROM mir_photos p LEFT JOIN mir_links l ON p.link_id = l.link_id ORDER BY p.created_at DESC LIMIT 1");
    $stmt->execute();
    $newest = $stmt->fetch();
}
